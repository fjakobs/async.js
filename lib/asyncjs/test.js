        ...
        */
        ...
        this.run().report().summary(function(err, passed) {
            ...
            /*if (typeof _$jscoverage != "undefined") {
                ...
                for (var file in cov) {
                    var hits = cov[file]
                    var sources = hits.source

                    newCov[file] = hits.map(function(hits, i) {
                        return {hits: hits, line: sources[i] }
                    })
                }
                
                fs.writeFile("coverage.json", JSON.stringify(newCov), function(err) {
...
exports.walkTestCases = function(paths, matchRe) {
    ...
    return async.concat.apply(async, walkers)
        ...
        .filter(function(file) {
            return file.stat.isFile() && file.name.match(matchRe || /_test\.js$/)
        })
        .get("path")
